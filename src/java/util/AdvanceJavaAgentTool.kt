...
object AdvanceJavaAgentTool {
     ...
     * @return
     */
    fun getHotDeployParam(project: Project): String? {

        if (SettingProperty.isHotDeploy(project)) {
            val hotDeployFile = File(getAdvanceLocation() + File.separator + SettingProperty.HOT_DEPLOY_AGENT + File.separator + "instant-invoke-agent.jar")
...
//                return """ -javaagent:"G:\workspaces\idea\维护\instant-invoke\instant-invoke-agent\target\instant-invoke-agent-1.0-SNAPSHOT.jar"="$availablePort" """
     ...
     * @return
     */
    fun getInstantInvokePlugin(project: Project): String? {
...
//                return """ -javaagent:"G:\workspaces\idea\维护\instant-invoke\instant-invoke-agent\target\instant-invoke-agent-1.0-SNAPSHOT.jar"="$availablePort" """
    ...
    fun getXrebelParam(project: Project): String? {

        if (SettingProperty.isXrebel(project)) {
            val xrebelFile = File(getAdvanceLocation() + File.separator + "xrebel.jar")
